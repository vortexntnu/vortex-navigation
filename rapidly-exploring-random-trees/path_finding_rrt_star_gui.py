...
def get_world_from_mouse(pos):
    """Return world coords (row, col) as floats, not snapped to center."""
    x, y = pos
    row = y / CELL_SIZE
    col = x / CELL_SIZE
    return (row, col)

def world_to_pixel(node):
    """Convert (row, col) world coords → pixel position."""
    r, c = node
    x = int(c * CELL_SIZE)
    y = int(r * CELL_SIZE)
    return (x, y)

def draw_grid(final_path=None):
    # Background
    for r in range(ROWS):
        for c in range(COLS):
    ...
    # Start & goal
    if start:
        pygame.draw.circle(screen, GREEN, world_to_pixel(start), 6)
    if goal:
        pygame.draw.circle(screen, RED, world_to_pixel(goal), 6)

    # Tree nodes
    for node in node_list:
        pygame.draw.circle(screen, (0, 0, 255), world_to_pixel(node), 4)

    # Tree edges
    for entry in tree:
        if entry["parent"] is not None:
            pygame.draw.line(screen, (255, 0, 0),
                             world_to_pixel(entry["parent"]),
                             world_to_pixel(entry["pos"]), 1)

    # Final path (yellow highlight)
    if final_path:
        for i in range(len(final_path) - 1):
            pygame.draw.line(screen, (255, 0, 0),
                             world_to_pixel(final_path[i]),
                             world_to_pixel(final_path[i+1]), 4)
...
def get_float_cell_from_mouse(pos):
    x, y = pos
    c, r = x // CELL_SIZE, y // CELL_SIZE
    # return center of the clicked cell in float coordinates
    return (r + 0.5, c + 0.5)
        ...
        if event.type == pygame.QUIT:
            ...
            if event.button == 1:  # left click
                if pygame.key.get_mods() & pygame.KMOD_SHIFT:
                    start = get_world_from_mouse(pygame.mouse.get_pos())
                    node_list.clear()
                    tree.clear()
                    ...
                    goal = get_world_from_mouse(pygame.mouse.get_pos())
